...
def just_rigid_(mem,coord,support,rxn,loads,l):
    if rxn==3:
...
def just_rigid_with_p_for_origid(mem,coord,support,rxn,loads,l,modified_loads,og_supp):
    if rxn==3:
...
def just_rigid_without_p_for_origid(mem,coord,support,rxn,loads,l,modified_loads,og_supp):
    if rxn==3:
        load_with_supports,support_soln,functional_support_soln=reaction_Truss.reaction_in_truss_new1_wop(coord,mem,support,loads)#have to edit support soln
        ckey=0
